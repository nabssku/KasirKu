...
class TransactionController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'payment_method'     => [($request->status === 'pending' ? 'nullable' : 'required'), 'string'],
    ...
    public function receipt(string $id): JsonResponse
    {
        ...
        $receipt = [
            ...
            'payment_method' => $transaction->payments->first()?->payment_method_name ?? $transaction->payments->first()?->payment_method ?? 'cash',
